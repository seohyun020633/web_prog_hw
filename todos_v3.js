...
async function loadTodos() {
    try {
        ...
        if (error.code === 'ENOENT') {
            console.log(`[Todo] ${todosFilePath} 파일이 존재하지 않아 빈 목록으로 시작합니다.`);
            return [];
        }
...
async function saveTodos(todosToSave) {
    try {
        await fs.writeFile(todosFilePath, JSON.stringify(todosToSave, null, 2), 'utf8');
...
async function startServer() {
    await loadTodos();
    app.listen(port, () => {
        console.log(`To-Do List v3 서버가 http://localhost:${port} 에서 실행 중입니다.`);
